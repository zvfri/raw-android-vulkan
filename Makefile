...
ANDROIDSRCS:= $(SRC) src/vkbootstrap/VkBootstrap.cpp src/androidnative/android_native_app_glue.c
...
CFLAGS+= -I$(NDK)/sysroot/usr/include -I$(NDK)/sysroot/usr/include/android -I$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/include -I$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/include/android -fPIC -DANDROIDVERSION=$(ANDROIDVERSION)
LDFLAGS += -lm -ldl -lvulkan -landroid -llog
...
CXXFLAGS := $(CFLAGS) -std=c++20 -static-libstdc++ -fvisibility-inlines-hidden
...
$(BUILD_DIR)/makecapk/lib/arm64-v8a/lib$(APPNAME).so : $(ANDROIDSRCS)
	mkdir -p $(BUILD_DIR)/makecapk/lib/arm64-v8a
	$(CXX_ARM64) $(CXXFLAGS) $(CFLAGS_ARM64) -o $@ $^ -L$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/lib/aarch64-linux-android/$(ANDROIDVERSION) $(LDFLAGS)

$(BUILD_DIR)/makecapk/lib/armeabi-v7a/lib$(APPNAME).so : $(ANDROIDSRCS)
	mkdir -p $(BUILD_DIR)/makecapk/lib/armeabi-v7a
	$(CXX_ARM32) $(CXXFLAGS) $(CFLAGS_ARM32) -o $@ $^ -L$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/lib/arm-linux-androideabi/$(ANDROIDVERSION) $(LDFLAGS)

$(BUILD_DIR)/makecapk/lib/x86/lib$(APPNAME).so : $(ANDROIDSRCS)
	mkdir -p $(BUILD_DIR)/makecapk/lib/x86
	$(CXX_x86) $(CXXFLAGS) $(CFLAGS_x86) -o $@ $^ -L$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/lib/i686-linux-android/$(ANDROIDVERSION) $(LDFLAGS)

$(BUILD_DIR)/makecapk/lib/x86_64/lib$(APPNAME).so : $(ANDROIDSRCS)
	mkdir -p $(BUILD_DIR)/makecapk/lib/x86_64
	$(CXX_x86) $(CXXFLAGS) $(CFLAGS_x86_64) -o $@ $^ -L$(NDK)/toolchains/llvm/prebuilt/$(OS_NAME)/sysroot/usr/lib/x86_64-linux-android/$(ANDROIDVERSION) $(LDFLAGS)
...
makecapk.apk : $(TARGETS) $(EXTRA_ASSETS_TRIGGER) $(BUILD_DIR)/AndroidManifest.xml $(KEYSTOREFILE)
	...
	cp -r main/assets/* $(BUILD_DIR)/makecapk/assets
	rm -rf $(BUILD_DIR)/temp.apk
	$(AAPT) package -f -F $(BUILD_DIR)/temp.apk -I $(ANDROID_JAR) -M $(BUILD_DIR)/AndroidManifest.xml -S main/res -A $(BUILD_DIR)/makecapk/assets -v --target-sdk-version $(ANDROIDTARGET)
	...
	/data/data/com.termux/files/usr/bin/zipalign -v 4 $(BUILD_DIR)/makecapk.apk $(BUILD_DIR)/$(APKFILE)
...
c :
	rm -rf $(BUILD_DIR)/AndroidManifest.xml $(BUILD_DIR)/temp.apk $(BUILD_DIR)/makecapk.apk $(BUILD_DIR)/makecapk $(BUILD_DIR)/$(APKFILE) $(BUILD_DIR)/$(APKFILE).idsig
